...
export type HTMLProps<E extends HTMLElement = HTMLElement, K extends keyof E = keyof E> = Partial<Record<`${'_' | '$'}${Exclude<K, symbol | `on${string}`>}`, any>> & Partial<Record<Extract<K, `on${string}`>, (e: Event) => void>> & {
