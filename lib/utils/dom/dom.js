...
function removeElements(target, delay) {
    if (delay === void 0) { delay = target[constants.REMOVE_DELAY]; }
    if (delay) {
        setTimeout(function () { return target.remove(); }, delay);
    }
    else {
        target.remove();
    }
...
function clear(target, delay) {
    target._children.forEach(function (target) { return removeElements(target, delay); });
    target._children = [];
}
